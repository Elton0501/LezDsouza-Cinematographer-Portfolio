    ...
    public class HelperService
    {
        ...
        public void RemoveOldUserVist()
        {
            using (var context = new ApplicationDbContext())
            {
                ...
                var ImagePagedata = Pagedata.Where(x => x.VisitPage == "Images").ToList();
                var OtherPagedata = Pagedata.Where(x => x.VisitPage != "Images").ToList();
                ...
                if (ImagePagedata != null && ImagePagedata.Count > 0)
                {
                    DateTime LatestImageDate = ImagePagedata.Max(x => x.VisitDateTime);
                    DateTime OldImageDate = LatestImageDate.AddDays(-7);
                    var oldImageData = ImagePagedata.Where(x => x.VisitDateTime < OldImageDate).ToList();
                    context.PageVisitCounts.RemoveRange(oldImageData);
                }

                if (OtherPagedata != null && OtherPagedata.Count > 0)
                {
                    DateTime LatestOtherDate = OtherPagedata.Max(x => x.VisitDateTime);
                    DateTime OldOtherDate = LatestOtherDate.AddDays(-30);
                    var oldOtherData = OtherPagedata.Where(x => x.VisitDateTime < OldOtherDate).ToList();
                    context.PageVisitCounts.RemoveRange(oldOtherData);
                }
